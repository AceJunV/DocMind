...
describe('documentParser', () => {
  it('parses UTF-8 TXT content and teaching fields', async () => {
    const file = makeFile(['课题：课堂观察记录\n学科：语文\n正文内容'], 'lesson.txt', { type: 'text/plain' })
    ...
    expect(result.teaching_plan?.topic).toBe('课堂观察记录')
    expect(result.teaching_plan?.subject).toBe('语文')
  })
  ...
  it('parses DOCX text content', async () => {
    const file = await createDocxFile(['课题：圆的面积', '教学目标：用于课堂评审', '正文内容'])

    const result = await parseDocument(file)

    expect(result.raw_content).toContain('圆的面积')
    expect(result.raw_content).toContain('用于课堂评审')
    expect(result.teaching_plan?.topic).toBe('圆的面积')
  })

  it('parses DOCX content even when the file extension is DOC', async () => {
    const file = await createDocxFile(['课题：兼容教案', '正文内容'], 'renamed.doc')

    const result = await parseDocument(file)

    expect(result.raw_content).toContain('兼容教案')
    expect(result.teaching_plan?.topic).toBe('兼容教案')
  })
