...
export default function ReviewDetailPage() {
  ...
  const teacherReviews = agentReviews.filter((ar) => {
    const agent = review.agents?.find((a) => a.id === ar.agent_id)
    return !agent?.category || agent.category === 'teacher'
  })
  const studentReviews = agentReviews.filter((ar) => {
    const agent = review.agents?.find((a) => a.id === ar.agent_id)
    return agent?.category === 'student'
  })
  const parentReviews = agentReviews.filter((ar) => {
    const agent = review.agents?.find((a) => a.id === ar.agent_id)
    return agent?.category === 'parent'
  })

  const radarDatasets = agentReviews.map((ar) => {
    const dimMap = new Map(ar.dimensions.map((d) => [d.name, d.score]))
    return {
      label: ar.agent_name,
      color: ar.agent_color,
      scores: TEACHING_DIMENSIONS.map((dim) => dimMap.get(dim) || 3.5),
    }
  })

  const avgDimScores = TEACHING_DIMENSIONS.map((dim) => {
    const scores = agentReviews.map((ar) => {
      const found = ar.dimensions.find((d) => d.name === dim)
      return found?.score || 3.5
    })
    return {
      name: dim,
  ...
  const buildReportMarkdown = () => {
    const lines = [
      ...
      `- **参与角色**：${agentReviews.map((ar) => ar.agent_name).join('、')}`,
      '',
      '---',
      '',
      '## 六维度评分汇总',
      ...
      '| 维度 | 平均分 | 最低 | 最高 |',
      '|------|--------|------|------|',
      ...avgDimScores.map((d) => `| ${d.name} | ${d.avg.toFixed(1)} | ${d.min.toFixed(1)} | ${d.max.toFixed(1)} |`),
      '',
      '---',
      '',
      '## 各角色评审详情',
      ...
      ...agentReviews.map((ar) => [
        ...
        '',
        '**维度评分：**',
        ...ar.dimensions.map((d) => `- ${d.name}: ${d.score.toFixed(1)}/5${(d as { comment?: string }).comment ? ` — ${(d as { comment?: string }).comment}` : ''}`),
        '',
        '**修改建议：**',
      ...
      ...(controversies.length > 0 ? [
        ...
        ...controversies.map((c) => [
          `### ${c.topic}`,
          ...c.opinions.map((op) => `- **${op.agent_name}**：${op.stance}`),
          '',
        ].join('\n')),
      ] : []),
    ]
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = `教研评审报告_${docTitle}_${new Date(review.created_at).toLocaleDateString('zh-CN')}.md`
  ...
  const renderAgentCards = (reviews: typeof agentReviews, title: string, icon: string) => {
    if (reviews.length === 0) return null
    return (
      <div>
        <h3 className="text-sm font-semibold text-gray-700 flex items-center gap-2 mb-3">
          <span className="text-base">{icon}</span> {title} ({reviews.length})
        </h3>
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    {review.agents?.find((a) => a.id === ar.agent_id)?.focusDimension && (
                      <span className="ml-1.5 text-[10px] rounded-full px-1.5 py-0.5 bg-amber-50 text-amber-600 border border-amber-200">
                        {review.agents?.find((a) => a.id === ar.agent_id)?.focusDimension}
                      </span>
                    )}
                  </div>
              ...
              <div className="space-y-1.5">
                {ar.dimensions.map((dim) => (
                  <div key={dim.name} className="flex items-center gap-2 text-xs">
                    <span className="w-16 text-gray-500 shrink-0">{dim.name}</span>
    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{agentReviews.length} 位角色参与评审</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {review.overall_score != null && (
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm text-center">
            ...
            <p className="text-sm text-gray-500 mt-3">
              {agentReviews.length} 位角色 · {consensus.length} 个共识 · {controversies.length} 个争议 · {allSuggestions.length} 条建议
            </p>
            {/* 六维度平均分 */}
            <div className="mt-4 space-y-1.5">
              {avgDimScores.map((d) => (
                <div key={d.name} className="flex items-center gap-2 text-xs">
                  <span className="w-16 text-gray-500 text-right shrink-0">{d.name}</span>
                  <div className="flex-1 h-2 rounded-full bg-gray-100">
                    <div
                      className="h-full rounded-full bg-primary-500 transition-all"
                      style={{ width: `${d.avg * 20}%` }}
                    />
                  </div>
                  <span className="text-gray-600 w-6 text-right font-medium">{d.avg.toFixed(1)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {radarDatasets.length > 0 && (
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm flex flex-col items-center justify-center">
            <p className="text-sm font-semibold text-gray-700 mb-3">六维度雷达图对比</p>
